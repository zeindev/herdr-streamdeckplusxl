...
- **The budget is real and forced choices.** About 359px of usable width means roughly 23 cells on the branch line at 28px and 32 on the readings line at 20px. Content is dropped rather than shrunk, so the reading furthest right gives way first. Attention, the ticket count and the pull request never do, and that is now structural rather than a happy accident of the current widths: each reading is marked required or droppable, and a droppable one is only kept if every required one still to come will also fit.
- **Reserving space means reserving *width*, not just a slot.** Each reserved reading is laid out at the width its real value will need rather than the width of the `?` standing in for it, so `-wl7` filling them in changes what they say and not where anything sits. The layout can promise a pull request four cells without costing the droppable reading, so its state has to be said in four — `OPEN`, `DRFT`, or a number. Saying it in more is allowed and simply spends the droppable reading, which is what droppable means.
- **A branch that will not fit loses its middle, not either end.** Cutting the tail would make `feature/auth/rewrite` and `feature/auth/revert` read identically; cutting the head would do the same to `alice/fix-login` and `bob/fix-login`. Only keeping both ends survives either, so the cut is `feature…the-token-store`.
- **The strip also had to answer for Herdr being gone.** When the connection is not live the counts would be whatever was last true rather than what is true, so they go and the channel says `OFFLINE` or `SYNCING` in their place. The **branch stays**: a branch does not change because Herdr died, and since the branch key left the keys in this same change, dropping it here would have left a workstream with no identity anywhere on the device. Each channel carries its own notice rather than the strip carrying one centrally, because a region only ever draws its own block — one central message would be legible on one region out of six.
