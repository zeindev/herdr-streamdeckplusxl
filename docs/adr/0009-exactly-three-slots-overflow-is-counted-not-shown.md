...
- **Reassignment is a hold, not a tap**, on the channel's identity key, and it is one gesture rather than a vocabulary: a hold moves that channel on to the next thing it could show. A channel holding a workstream lets it go; an empty one takes in the first workstream waiting, and holding again offers the next, so the developer chooses *which* workstream a channel gets rather than being given the longest-waiting one. "Nothing" is always in the cycle, so a channel can always be cleared and holding repeatedly returns to where it started. A hold cannot happen by brushing a key, which is the friction this decision asks for.
- **A workstream taken off the device stays off it.** This is not decoration: automatic assignment runs on every snapshot, which is many times a minute, so without it releasing a channel would be undone before the developer let go of the key. Released workstreams are remembered as detached, are passed over by automatic assignment, and are forgotten once they no longer exist.
