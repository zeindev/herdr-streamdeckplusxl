...
**Focus** (column 0). A tap sends `workspace.focus`. Whether Herdr raises its own window without stealing operating-system focus is asserted from the API shape — `workspace.focus` takes only a `workspace_id`, giving the plugin nothing to ask for OS focus with — the same caveat ticket `-0vd.5` already carries for `pane.focus`, and not independently verified here for the same reason: doing so would move focus in the developer's own running session.
